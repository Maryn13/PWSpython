...
class Env:
    ...
    def reset(self):
        ...
        self.food.instantiate(x =100, y =100)
        ...
        if self.RETURN_IMAGES:
            ...
            observation = [self.player.loc[0]-self.food.loc[0], self.player.loc[1]-self.food.loc[1],  self.player.angvel, self.player.angacc]

        return observation

    def step(self, action, dcr):
        self.episode_step += 1
        self.player.action(action)

        for i in range(0, int(dcr/self.player.dt)):
        ...
        if self.RETURN_IMAGES:
            ...
            new_observation = [self.player.loc[0]-self.food.loc[0], self.player.loc[1]-self.food.loc[1],  self.player.angvel, self.player.angacc]


        if self.player == self.food:
            reward = self.FOOD_REWARD
        elif self.episode_step >= 500:
            reward = 25/(np.abs(0.05*self.food.x - 0.05*self.player.loc[0]) + np.abs(0.05*self.food.y - 0.05*self.player.loc[1]))
        else:
            reward = -self.MOVE_PENALTY

        done = False
        if reward == self.FOOD_REWARD or self.episode_step >= 500:
            done = True
...
class DQNAgent:
    ...
    def train(self, terminal_state, step):
        ...
        self.model.train(np.array(X), y, epochs=1, batch_size=MINIBATCH_SIZE, show_stats=False)
    ...
    if SHOW_PREVIEW and not episode % AGGREGATE_STATS_EVERY:
        ...
        while running:
            ...
            while not done:
                ...
                new_state, reward, done = env.step(action, agent.inputrate)
